...
#         return False
    ...
    def make_move(self,move):
        # move is given in form 'H'/ 'V' for horizontal vertical, starting coordiate, array of tiles
        if move[0] == 'H':
            for i in range(len(move[2])):
                self.grid[move[1] + Coordinates(i,0)] = move[2][i]
        else : 
            assert move[0] == 'V'
            for i in range(len(move[2])):
                self.grid[move[1] + Coordinates(0,i)] = move[2][i]
...
def valid_move_with_count(grid,tile_number):
    return [i for i in all_possible_moves(grid) if len(i[2]) <= tile_number]
    
    

class GameNode:
    def __init__(self,g, tile_number):
        self.grid = g #Grid Class
        self.tile_number  = tile_number
        self.successors = [] #array of GameNode

        
def make_successors(gamenode):
    
    moves = valid_move_with_count(gamenode.grid,gamenode.tile_number)
    for i in moves:
        gamenode.successors.append( GameNode ( gamenode.grid.make_move(i) , gamenode.tile_number - len(i[2]) )  )
    return gamenode
# def winning(gamenode):
    
#     if gamenode.grid.is_closed():
#         return True
...
def winning(gamenode):
    if not gamenode.grid.is_closed() and gamenode.tile_number == 0:
        return True
    
    if gamenode.grid.is_closed():
        return False
    
    gamenode = make_successors(gamenode)
    
    
    for i in gamenode.successors:
        if i.grid.is_closed():
            return True
...
g = Grid({})
g.grid[Coordinates(0,0)] = Tile(LEFT, RIGHT)
g.grid[Coordinates(1,0)] = Tile(LEFT, RIGHT) 
